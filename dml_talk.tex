...
\begin{frame}{Beyond PLR: ATE, ATTE, PLIV, LATE}
  \begin{itemize}
    \item \textbf{ATE / ATTE under unconfoundedness}:
      \begin{itemize}
        \item Orthogonal scores combining outcome regression and propensity score
        \item Same pattern: residuals + cross-fitting
      \end{itemize}
    \item \textbf{Partially linear IV model}:
      \[
      ...
      \]
      \begin{itemize}
        \item $Z$ affects $Y$ only via $D$ given $X$ (exclusion)
        \item DML uses an orthogonal score involving $(Y - \theta D - g_0(X))(Z - m_0(X))$
      \end{itemize}
    \item \textbf{LATE with binary $D,Z$}:
      \begin{itemize}
        \item Orthogonal scores for Wald-type estimands, with ML nuisance functions for $m_0,p_0,\mu_0$.
      \end{itemize}
    \item In all cases: low-dimensional $\theta_0$, high-dimensional $\eta_0$, orthogonal score + ML + cross-fitting.
  \end{itemize}
\end{frame}
